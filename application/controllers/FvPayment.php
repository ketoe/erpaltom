<?php
class FvPayment extends CI_Controller {
	...
	function __construct() {
     ...
     function addFvPayment () {
      ...
      if ($this->form_validation->run() == TRUE) {
        ...
        $this->M_fvPayment->addFvPayment($data);
     ...
     public function deleteFvPayment($id, $order) {
      if ($this->M_user->permission > 0) {
        $this->M_fvPayment->deleteFvPayment($id);
    ...
    public function edit ($id, $order = null) {
      $fv = $this->M_fvPayment->getIdFvPayment($id);
      ...
      $this->load->view('general/top');
      $this->load->view('fvPayment/edit',$data);
    }
